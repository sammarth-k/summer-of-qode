...
A ``while`` loop takes a condition in the beginning of the loop and checks it. If the the condition evaluates to ``True``, then the code inside will run, but if it isn't, the loop ends. The condition is checked each time before the loop runs. The syntax is as follows:
...
An infinite loop is a loop that _never_ breaks, or never stops. Remember to watch out for these, since they occur when you forget to add a mechanism to stop the loop. However, if you want to make one, there are several ways you can do that.
...
```
We cannot display the output since it will run infinitely, but it would print "Hi" over and over again.
```
...
All of these programs output the same thing, since the condition in the ``while`` loop always evaluates to ``True ``and never changes.
...
A ``for`` loop has a variable known as the control variable, which can be used in the loop and hence the loop doesn't require incrementing. The syntax is as follows

```python
for i in 'hello': # i is the control variable
  print(i)
```
...
As the loop goes through the string, the value of ``i`` will be each element of the string, which we can use like a variable in the loop. So, in the first iteration, ``i = "h"``, in the second ``i = "e"`` and so on.

The above example is an example of how we can **iterate through a sequence**. A sequence can be either a string, list, tuple, etc (you will learn about lists later, the other data types are for future learning). When you iterate over a sequence using the ``for`` loop, for the nth iteration, the control variable assumes the value of the nth element in the sequence. For a string, each character is considered as a value in a sequence (including white spaces).

We iterate over a range of numbers in the loop as well, using the ``range()`` function.
...
The first number is **always** included.

You can also enter a third parameter to change the increment value, known as the step value in this case
...
The step value is the increment here, and the value of ``i`` increases by that value every time. Negative values make the loop go in the reverse order.

To better understand how these things work, we encourage you to play around with the code.
